...
- **`bitcoindIpcDataDir` is the socket's parent directory, and the mount target is the socket itself.** Upstream appends `[<network>/]node.sock` to `data_dir`, so `ipcSocketMountpoint()` has to reproduce that mapping exactly; a mismatch fails at connect time, not at parse time.
