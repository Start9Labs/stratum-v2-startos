...
- **A dependency mount can only be a directory.** `MountTarget.filetype` is `#[serde(skip_deserializing)]` in start-core, so the OS always defaults it to `Directory` and the SDK's `type: 'file'` option is inert here; binding a socket onto the directory it creates fails with `mount exited with exit status: 32`. Hence the `link-ipc-socket` oneshot: mount Bitcoin's `ipc` **directory**, then symlink `node.sock` to it from a `data_dir` on our own volume. Don't collapse that back into a direct socket mount without checking whether the OS has started honoring `filetype`.
